  ...
  <q-layout view="hhh LpR fFf">
    ...
    >
      <div style="display: grid; padding: 20px; position: relative">
        <label class="label-inventory">Inventory</label>
        <label class="label-inventory-version">Version 1.0.0</label>
        <q-btn
          flat
          round
          class="c-btn-close"
      ...
      <div class="criteria-menu">
        <template v-for="(row_menu, index_menu) in listMenu" :key="index_menu">
          <template
            v-for="(row_main, index_main) in row_menu.childItem"
            :key="index_main"
          >
            <q-expansion-item
            ...
            >
              <template
                v-for="(row_child, index_child) in row_main.childItem"
                :key="index_child"
              >
                <q-item
...
export default {
  setup() {
    ...
    const $q = useQuasar();
    const { listMenu, getMenu } = useUtil.useMenu();
    ...
    const findMenu = (findNode, name) => {
      for (let i = 0; i < findNode.length; i++) {
        if (findNode[i].name == name) {
          ...
          findNode[i].childItem !== undefined &&
          findNode[i].childItem.length > 0
        ) {
    ...
    const onRedirect = (name, params) => {
      ...
      router.push({ name: currentRow.name, params: params }).then(() => {
        hideLoading();
      });
    };
...
.criteria-main {
  ...
  .label-inventory-version {
    font-family: "ThaiBevSansNew-Medium";
    font-size: 12px;
  }
  .criteria-menu {
    font-family: "ThaiBevSansNew-Black";
    font-size: 16px;
    padding-left: 5px;
